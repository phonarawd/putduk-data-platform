-- 이메일 인증된 회원은 근무 가능. 차단 계정은 건드리지 않는다.
create or replace function private.putduk_activate_confirmed_user()
returns trigger
language plpgsql
security definer
...
begin
  if new.email_confirmed_at is not null
     and (tg_op = 'INSERT' or old.email_confirmed_at is distinct from new.email_confirmed_at) then
...
revoke all on function private.putduk_activate_confirmed_user() from public, anon, authenticated;
grant execute on function private.putduk_activate_confirmed_user() to service_role;

drop trigger if exists putduk_activate_confirmed_user on auth.users;
create trigger putduk_activate_confirmed_user
after insert or update of email_confirmed_at on auth.users
for each row
execute function private.putduk_activate_confirmed_user();

-- 체험 수당 3천은 KYC 전에 운영 경로로 1회만 신청 가능.
...
declare
  ...
  v_ops boolean := false;
begin
  select status, kyc_status, trial_consumed_at
    into v_status, v_kyc, v_trial
...
revoke all on function private.putduk_withdraw_identity_ok(uuid, numeric, boolean) from public, anon, authenticated;
grant execute on function private.putduk_withdraw_identity_ok(uuid, numeric, boolean) to service_role;
...
begin
  ...
  perform private.putduk_withdraw_identity_ok(p_user_id, p_amount, p_include_principal);
