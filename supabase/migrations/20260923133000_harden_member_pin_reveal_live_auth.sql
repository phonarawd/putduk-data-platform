...
alter function public.putduk_security_pin_get(uuid)
  rename to putduk_security_pin_get_impl;

alter function public.putduk_security_pin_upsert(uuid,text,integer,timestamptz)
  rename to putduk_security_pin_upsert_impl;

alter function public.putduk_security_pin_set_failures(uuid,integer,timestamptz)
  rename to putduk_security_pin_set_failures_impl;

alter function public.putduk_deposit_reveal_tokens_lock(uuid,text,text)
  rename to putduk_deposit_reveal_tokens_lock_impl;

alter function public.putduk_deposit_reveal_token_insert(uuid,text,text,integer,timestamptz)
  rename to putduk_deposit_reveal_token_insert_impl;
...
begin
  perform private.putduk_assert_live_member(p_user_id);
  perform public.putduk_deposit_reveal_token_insert_impl(
    p_user_id,p_token_hash,p_scope,p_catalog_version,p_expires_at
  );
end;
$function$;
