-- 출근 시작 이벤트는 AFTER INSERT에서만 남긴다.
--
-- 18110000·18140000이 하루 한도를 넣으면서, 이미 고친 BEFORE INSERT의
-- task_events 기록을 다시 넣었다. task_runs 행이 생기기 전에 FK(23503)가 깨져
-- 회원이 "출근 기록을 저장하지 못했어요"를 본다.
-- 기존 마이그레이션은 건드리지 않고, 한도·잠금 로직은 그대로 둔 채 이벤트만 옮긴다.
...
comment on function private.prepare_putduk_task_run() is
  '회원 업무 시작 시 공개 카드/배정·등급 접근·동시진행 1건·노드별 하루 한도·회원 하루 총 횟수(등급→예외→추가)·보상·잠금을 서버가 확정한다. 출근 이벤트는 AFTER INSERT에서만 남긴다.';
...
begin
  ...
  order by a.started_at desc nulls last, a.updated_at desc
  limit 1;
