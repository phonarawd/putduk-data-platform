(() => {
  ...
  const storageKey = 'putduk-demo-state-v1';
  ...
  function renderMemberDashboard() {
    ...
    return `
      ...
      <section class="grid-hero">
        ...
        <div class="id-card"><div class="id-card-top"><div><div class="id-card-kicker">퍼뜩 멤버십 카드</div><div class="id-card-title">우수 파트너</div></div><div class="id-chip"></div></div><div class="id-number">${esc(publicId())}</div><div class="id-footer"><span>${esc(profileName())}</span><span>${authState.session ? '인증 계정' : '미리보기 카드'}</span></div><div style="display:flex;align-items:center;gap:7px;margin-top:17px;color:#b9e8d2;font-size:11px">${icon('sparkles', 14)} 활동으로 등급이 올라가요</div></div>
      </section>
  ...
  function renderAuthModalLive() {
    ...
    const connectionCopy = supabaseClient
      ...
      : '현재는 미리보기 모드입니다. 운영 전 Supabase 인증·이메일 발송 설정을 연결하면 실제 계정으로 전환됩니다.';
  ...
  async function sendPasswordReset() {
    ...
    if (!supabaseClient) { showToast('미리보기 모드에서는 비밀번호 재설정 메일을 보내지 않아요.', 'info'); return; }
  ...
  async function submitSignup(event) {
    ...
    if (!supabaseClient) {
      closeModal();
      showToast('✅ 가입 정보가 접수됐어요. 실제 계정 연결 전 미리보기 카드로 확인할 수 있어요.', 'success');
      return;
    }
    const metadata = {
  ...
  async function submitLogin(event) {
    ...
    const form = event.target;
    if (!form.reportValidity()) return;
    if (!supabaseClient) { showToast('현재는 미리보기 모드예요. 운영 인증 연결 후 로그인할 수 있어요.', 'info'); return; }
  ...
  function handleClick(event) {
    ...
    if (action === 'withdraw-request') { closeModal(); showToast(authState.session ? '출금 API 연결 전에는 요청을 만들지 않았어요. 잔액은 변경되지 않았습니다.' : '미리보기에서는 출금 요청을 만들지 않아요.', 'info'); return; }
    ...
    if (action === 'review-demo') { showToast('검수 상세 화면을 준비했어요. 최종 확정 전 내용을 확인하세요.', 'info'); return; }
    if (action === 'finance-demo') { showToast('입출금 요청의 증빙과 본인확인 상태를 함께 확인하세요.', 'info'); return; }
