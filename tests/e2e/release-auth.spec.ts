  ...
  test(`로그인 오류 분류: ${scenario.name}`, async ({ page }) => {
    await page.route('**/auth/v1/token?grant_type=password**', async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({ error: scenario.code, error_description: scenario.message, error_code: scenario.code, code: scenario.code, msg: scenario.message, message: scenario.message })
      });
    });
